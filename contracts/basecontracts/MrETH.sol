...
contract MrETH is ERC20, Ownable {
    ...
    address public marketingAddress =
        0x70997970C51812dc3A010C7d01b50e0d17dc79C8; //Marketing Address
    address public devAddress = 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266; //Dev Address
    address public mrETHPoolAddress;
    ...
    constructor() ERC20("Mr ETH", "MRETH") {
        //Mint tokens
        ...
        exemptFee[mrETHPoolAddress] = true;
        ...
        allowedTransfer[mrETHPoolAddress] = true;
    }
    ...
    function _transfer(
        ...
        require(amount > 0, "MrETH: Transfer amount must be greater than zero");
        require(
        ...
        if (!exemptFee[sender] && !exemptFee[recipient]) {
            require(tradingEnabled, "MrETH: Trading disabled");
        }

        if (sender == pair && !exemptFee[recipient] && !_liquidityMutex) {
            require(amount <= maxBuyLimit, "MrETH: Exceeding max buy limit");
            require(
                balanceOf(recipient) + amount <= maxWalletLimit,
                "MrETH: Exceeding max hold limit"
            );
        }
        ...
        if (
            ...
            require(amount <= maxSellLimit, "MrETH: Exceeding max sell limit");

            if (recipient != pair) {
                require(
                    balanceOf(recipient) + amount <= maxWalletLimit,
                    "MrETH: Exceeding max wallet limit"
                );
            }

            if (coolDownEnabled) {
                uint256 timePassed = block.timestamp - _lastSell[sender];
                require(timePassed >= coolDownTime, "MrETH: Cooldown enabled");
        ...
        if (
            !exemptFee[sender] &&
            !exemptFee[recipient] &&
            block.number <= genesis_block + 9
        ) {
            require(
                recipient != pair,
                "MrETH: Sells not allowed in first 9 blocks"
            );
        }
    ...
    function handle_fees(
        ...
        if (tokenBalance >= tokenLiquidityThreshold) {
            ...
            // Token distribution
            uint256 liquidityTokens = (swapTaxes.liquidity * tokenBalance) /
                feeswap;
            uint256 marketingTokens = (swapTaxes.marketing * tokenBalance) /
                feeswap;
            uint256 devTokens = (swapTaxes.dev * tokenBalance) / feeswap;
            uint256 mrETHPoolTokens = tokenBalance -
                liquidityTokens -
                marketingTokens -
                devTokens;
            ...
            uint256 poolAmt = (mrETHPoolTokens * afterBalance) / toSwap;
            if (poolAmt > 0) {
                payable(mrETHPoolAddress).sendValue(poolAmt);
            }
        }
    }
    ...
    function setSellTaxes(Taxes calldata _newSellTaxes) external onlyOwner {
        require(
            _newSellTaxes.liquidity +
                _newSellTaxes.marketing +
                _newSellTaxes.dev +
                _newSellTaxes.MrETHPool <=
                10,
            "MrETH: Total sell taxes above 10%"
        );
    ...
    function setMrETHPoolAddress(
        address _newMrETHPoolAddress
    ) external onlyOwner {
        require(
            _newMrETHPoolAddress != address(0),
            "MrETH: MrETh Pool is the zero address"
        );
        mrETHPoolAddress = _newMrETHPoolAddress;
    }
    ...
    function setMaxTxLimit(
        ...
        require(
            _maxBuy > 99 && _maxSell > 99,
            "MrETH: Max buy and sell must be above 99."
        );
    ...
    function setMaxWalletlimit(uint256 _amount) external onlyOwner {
        require(_amount > 99, "MrETH: Max wallet limit must be above 99.");
